...
**Result:** `/ask` is implemented with question validation, FAISS retrieval, grounded answer generation via the configured LLM provider, and end-to-end verification against a real indexed PDF. The current behavior scopes retrieval to a specific uploaded PDF via `document_id` when multiple indexes exist, and returns citation-ready context metadata including page numbers and chunk indexes.
