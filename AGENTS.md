...
当前项目处于第 6 阶段：前后端主链路已经打通，文档去重、页码引用、聊天式前端和流式回答都已完成。用户现在可以上传 PDF、复用重复文档索引、围绕当前文档连续提问，以流式方式查看回答生成过程，并在每条 AI 消息下查看引用页码与片段。当前目标是继续优化聊天体验、多文档切换与细节可用性。
...
- `/ask/stream` 的 `done` 事件也会返回完整 `answer`、`contexts` 和 `citations`，不要让流式链路丢失这些字段
...
当前已落地的聊天体验约束：
- 前端默认通过流式接口渲染回答，不要无故退回到只等完整 JSON 再展示
- 发送新问题时聊天区应自动跟随到底部，但用户手动滚动后，后续流式输出不能强制抢回滚动位置
- 右侧聊天区、输入区和顶部摘要区已经做过紧凑化处理，后续改动时优先维持首屏信息密度
...
- 当前前端已经接入流式回答，不要把用户体验退回到“整段回答生成完再一次性出现”
