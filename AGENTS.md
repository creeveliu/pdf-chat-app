...
当前项目处于第 3 阶段：后端 PDF 上传、文本解析、chunking、embedding 和 FAISS 索引落盘已完成。当前目标是补齐检索接口和问答接口，并把前端上传结果接入展示层。
...
1. 前端上传组件与请求封装
2. 上传结果展示和错误提示
3. query embedding + FAISS top-k 检索
4. 文档处理结果到问答接口的串联

后端上传与索引能力已经存在，不要重复改写同一套上传/建索引逻辑，优先在现有服务基础上扩展。
